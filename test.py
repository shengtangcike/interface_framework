...
for test_sheet_name in get_test_case_sheet_names(test_data_excel_path):
    flag = True   #是否全部通过的标志
    test_cases_wb.set_sheet_by_name(test_sheet_name[1])      #工作簿名称
    test_cases = test_cases_from_test_data_sheet(test_data_excel_path,test_sheet_name[1])
    for test_case in test_cases:
        start_time = time.time()
        #(1, 'register', '{"username":"testman${unique_num1}", "password":"123456789abc","email":"sed@qq.com"}', '"code": "00"', 'userid||userid": (\\d+)')
        ...
        try:
            ...
            test_results_for_html_report.append((r.url, send_data, r.text, int((end_time - start_time) * 1000), test_case[3], "成功"))
        except AssertionError:
            test_cases_wb.write_cell_value(int(test_case[0] + 1),  test_data_test_result_col_no+1,"失败")
            test_results_for_html_report.append((r.url, send_data, r.text, int((end_time - start_time) * 1000), test_case[3], "失败"))
            flag = False
        except:
            test_cases_wb.write_cell_value(int(test_case[0] + 1),  test_data_test_result_col_no+1, "失败")
            test_results_for_html_report.append((r.url, send_data, r.text, int((end_time - start_time) * 1000), test_case[3], "失败"))
